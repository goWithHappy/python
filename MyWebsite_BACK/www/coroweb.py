...
def get(path):
    ...
    '''
    def decorator(func):    #这样一个函数通过@get（）装饰就附加了url信息
        @functools.wraps(func)
        def wrapper(*args,**kw):
            return func(*args,**kw)
        wrapper__method__='GET'
        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            ...
            if request.method == 'POST':
                ...
                if ct.startswith('application/json'):
                    ...
                    kw = params
                #检查“content_type”类型是否为“application/x-www(first)-form-urlencoded”或“multipart/form-data”开头的字符串类型：
                elif ct.startswith('application/x-www(first)-form-urlencoded') or ct.startswith('multipart/form-data'):
                    #读取请求内容的POST参数：
